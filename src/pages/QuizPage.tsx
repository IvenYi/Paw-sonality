...
const QuizPage = ({ petType, onComplete, isDeep = false }: QuizPageProps) => {
  ...
  const finalizeResult = (finalScores: Scores) => {
    const E_or_I = finalScores.E > finalScores.I ? 'E' : 'I';
    const S_or_N = finalScores.S > finalScores.N ? 'S' : 'N';
    const T_or_F = finalScores.T > finalScores.F ? 'T' : 'F';
    const J_or_P = finalScores.J > finalScores.P ? 'J' : 'P';
    let finalMBTI = `${E_or_I}${S_or_N}${T_or_F}${J_or_P}`;
    if (isDeep) {
      const A_or_T = finalScores.A > finalScores.T_ ? 'A' : 'T';
      finalMBTI = `${finalMBTI}-${A_or_T}`;
    }
    onComplete(finalMBTI, finalScores);
  };
                  ...
                  return (
                    <motion.button
                      ...
                      onClick={(e) => {
                        ...
                        handleOptionClick(option.dimension, option.weight, index);
                      }}
